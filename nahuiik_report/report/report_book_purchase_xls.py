...
class reportPurchaseXls(models.AbstractModel):
    _name = 'report.nahuiik_report.report_libro_compras_invoice_xls'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for purchase in lines:
            sheet = workbook.add_worksheet(_('Libro compras ' + str(cont)))
            cont+=1
            ...
            sheet.merge_range('B2:I2', company_name, format11)
            
            sheet.write(3, 1, _('Shopping book'), format23)
            ...
            sheet.write(6, 2, self.change_format(purchase.start_date), format24)
            sheet.write(6, 3, _('End Date'), format23)
            sheet.write(6, 4, self.change_format(purchase.end_date), format24)

            sheet.write(7, 1, _('Total Purchases'), format23)
            sheet.write(7, 2, purchase.total_incoming, format5)
            sheet.write(7, 3, _('Taxable Purchases'), format23)
            sheet.write_number(7, 4, purchase.total_gravado, format5)

            sheet.write(8, 3, _('Total taxes'), format23)
            sheet.write_number(8, 4, purchase.total_impuesto, format5)
            
            pos = 11
            sheet.write(10, 1, _('Date'), format21)
            sheet.write(10, 2, _('Supplier'), format21)
            sheet.write(10, 3, _('RTN'), format21)
            sheet.write(10, 4, _('CAI'), format21)
            sheet.write(10, 5, _('Invoice Number'), format21)
            sheet.write(10, 6, _('Subt Total'), format21)
            sheet.write(10, 7, _('Taxes'), format21)
            sheet.write(10, 8, _('Total'), format21)
            for invoice in purchase.out_invoice_ids:
                ...
                sheet.write(pos, 4, invoice.invoice_id.cai_shot_compras or '', format22)
