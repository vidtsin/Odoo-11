...
class Check(models.Model):
    ...
    check_lines = fields.One2many("banks.check.line", "check_id", "Detalle de cheques", required=True, copy=True)
    ...
    is_for_payroll  =   fields.Boolean(string="Para Planilla")
    ...
    def action_validate(self):
        ...
        if not self.cheque_anulado:
            ...
            #check duplication of sequence - code start
            find_ids = self.env['banks.check'].search([('number', '=', self.number),('id', '!=', self.id),
                                                   ('state', '=', 'validated')])
            if find_ids:
